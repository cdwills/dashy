defmodule DashyWeb.DraggableLive do
  ...
  def handle_event("dropped", %{"draggedId" => dragged_id, "dropzoneId" => drop_zone_id,"draggableIndex" => draggable_index}, %{assigns: assigns} = socket) do
    drop_zone_atom = get_drop_zone_atom(drop_zone_id)
  ...
  def render(assigns) do
    ...
    <%= live_component @socket, DashyWeb.DropZoneComponent,
    ...
    <div phx-hook="Drag" id="drag">
      <div class="dropzone grid grid-cols-4 gap-3" id="pool">
        <%= for %{text: text, id: id} <- @pool do %>
          <div draggable="true" id="<%= id %>" class="draggable p-4 bg-blue-700 text-white">
            <%= text %>
          </div>
        <% end %>
      </div>
